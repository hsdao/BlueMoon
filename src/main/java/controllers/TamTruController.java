...
public class TamTruController implements Initializable {
    ...
    @FXML private TableColumn<TamTruTamVang, String>     colPhong;
    ...
    /** Cache id nhân khẩu → mã phòng (ma_ho của hộ khẩu chứa nhân khẩu đó) */
    private Map<Integer, String> nhanKhauPhong = new HashMap<>();
    ...
    private void buildNhanKhauCache() {
        nhanKhauCache.clear();
        nhanKhauPhong.clear();
        // Map id hộ khẩu -> mã phòng để biết nhân khẩu đang ở phòng nào
        Map<Integer, String> hoKhauMaHo = new HashMap<>();
        for (HoKhau hk : hoKhauDAO.getAllHoKhau()) hoKhauMaHo.put(hk.getId(), hk.getMaHo());

        for (NhanKhau nk : nhanKhauDAO.getAll()) {
            nhanKhauCache.put(nk.getId(), nk.getHoTen());
            nhanKhauPhong.put(nk.getId(), hoKhauMaHo.getOrDefault(nk.getHoKhauId(), "—"));
        }
    }
    ...
    private void setupColumns() {
        ...
        // Cột Phòng: tra mã phòng từ nhân khẩu
        colPhong.setCellValueFactory(cellData -> new javafx.beans.property.SimpleStringProperty(
                nhanKhauPhong.getOrDefault(cellData.getValue().getNhanKhauId(), "—")));
