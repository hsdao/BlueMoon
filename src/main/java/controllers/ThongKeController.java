...
public class ThongKeController implements Initializable {
    ...
    void handleXuatExcel(ActionEvent event) {
        if (tvThongKe.getItems().isEmpty()) {
            ...
            alert.setContentText("Khong co du lieu de xuat!");
            alert.showAndWait();
            return;
        }
        javafx.stage.FileChooser fc = new javafx.stage.FileChooser();
        fc.setTitle("Luu File Excel");
        ...
        if (file != null) {
            try {
                ...
                String title = cmbKhoanThu.getValue() != null
                        ? "Bao Cao Thong Ke: " + cmbKhoanThu.getValue().getTenKhoan()
                        : "Bao Cao Thong Ke";
                service.exportThongKe(tvThongKe.getItems(), title, file);
                Alert ok = new Alert(Alert.AlertType.INFORMATION);
                ok.setTitle("Thanh cong");
                ok.setHeaderText(null);
                ok.setContentText("File Excel da duoc xuat:\n" + file.getAbsolutePath());
                ok.showAndWait();
            } catch (Exception e) {
                e.printStackTrace();
                Alert err = new Alert(Alert.AlertType.ERROR);
                err.setContentText("Loi xuat Excel: " + e.getMessage());
                err.showAndWait();
            }
        }
    }
