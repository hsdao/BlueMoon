...
public class ExportExcelService {
    ...
    public void exportThongKe(List<ThongKeModel> data, String reportTitle, File file) throws Exception {
        try (Workbook wb = new XSSFWorkbook()) {
            ...
            String[] headers = {
                "Ma Ho Khau", "Ten Chu Ho", "So Tien Nop (VND)",
            ...
            for (ThongKeModel row : data) {
                ...
                Cell tienCell = dataRow.createCell(2);
                tienCell.setCellValue(soTien != null ? soTien.doubleValue() : 0);
                dataRow.createCell(3).setCellValue(
                    row.getNgayNop() != null ? row.getNgayNop().format(DATE_FMT) : "");
                dataRow.createCell(4).setCellValue(row.getTrangThai() != null ? row.getTrangThai() : "");
                dataRow.createCell(5).setCellValue(row.getGhiChu()    != null ? row.getGhiChu()    : "");
                for (int c = 0; c < 6; c++) dataRow.getCell(c).setCellStyle(dataStyle);
            }
